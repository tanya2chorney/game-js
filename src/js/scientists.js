...
// Функція для рендерингу результатів у вигляді карток
function renderScientists(scientists) {
  container.innerHTML = '';
  scientists.forEach(sci => {
    const div = document.createElement('div');
    div.classList.add('scientist');
    div.innerText = `${sci.name} ${sci.surname}     ${sci.born} - ${sci.dead}`;
    container.appendChild(div);
  });
}

function showMessage(message) {
  container.innerHTML = `<div class="scientist">${message}</div>`;
}

// Події для кожної кнопки
...
document.getElementById('btnSumYears').onclick = function () {
  const totalYears = scientists.reduce(
    (sum, sci) => sum + (sci.dead - sci.born),
    0
  );
  showMessage(`Сума років: ${totalYears}`);
};
...
document.getElementById('btnLatestBorn').onclick = function () {
  const latest = scientists.reduce((prev, curr) =>
    curr.born > prev.born ? curr : prev
  );
  showMessage(`Найпізніше народжений: ${latest.name} ${latest.surname}`);
};
...
document.getElementById('btnFindEinstein').onclick = function () {
  ...
  showMessage(`Альберт Ейнштейн народився у ${einstein.born} році.`);
};
...
document.getElementById('btnMaxMinLived').onclick = function () {
  const max = scientists.reduce((a, b) =>
    b.dead - b.born > a.dead - a.born ? b : a
  );
  const min = scientists.reduce((a, b) =>
    b.dead - b.born < a.dead - a.born ? b : a
  );
  showMessage(
    `Найдовше прожив ${max.name} ${max.surname}, найменше - ${min.name} ${min.surname}`
  );
};
...
document.getElementById('btnAll19').onclick = function () {
  const allWorkedIn19 = scientists.every(
    sci => sci.born >= 1801 && sci.born < 1901
  );
  showMessage(`Всі працювали в 19 ст.: ${allWorkedIn19}`);
};
